...
export async function generateImage(
  ...
  const imageUrl = await page.evaluate(
    async (prompt: string, imageId: string) => {
      ...
      const imageInput = (await waitForAndQuerySelector(
        "#imagegen-input",
      )) as HTMLElement;
      ...
      let generatedImageElement = document.querySelector(
        'img[alt="Generated Image"]',
      ) as HTMLImageElement;
      while (generatedImageElement && generatedImageElement.src) {
        await sleepBrowser(10);
        generatedImageElement = document.querySelector(
          'img[alt="Generated Image"]',
        ) as HTMLImageElement;
        ...
        if (errorToast) {
          ...
          return {
            error:
              "Error generating image. Filters don't really love your prompt...",
          };
        }
      }
