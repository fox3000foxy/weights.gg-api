...
export class LoraService {
    ...
    public async addLora(loraName: string, page: Page): Promise<boolean> {
        try {
            ...
            const loraFound = await page.evaluate(async () => {
                ...
                async function waitForAndQuerySelector(selector: string, timeout = 5000): Promise<Element | null> {
    ...
    public async searchLoras(loraName: string, page: Page): Promise<LoraResult[]> {
        await page.evaluate(async (loraName: string) => {
            const sleepBrowser = (ms: number) => new Promise(r => setTimeout(r, ms));
        ...
        const loraList = await page.evaluate(async () => {
            ...
            if (listElement) {
                ...
                return Array.from(listElement.children).map(child => ({
                    name: (child.querySelector("h3") as HTMLElement)?.innerText.trim() || '',
