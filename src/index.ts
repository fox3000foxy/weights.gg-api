...
import LoraSearchProcessor from './processors/loraSearchProcessor';
...
// --- Queue Processors ---
const processImageJob = async (job: any, generationPage: Page) => {
    await imageProcessor.processImage(job, generationPage);
};

const processLoraSearchJob = async (job: any, loraSearchPage: Page) => {
...
async function main() {
    ...
    await Promise.all([
        puppeteerService.onStart(puppeteerService.generationPage as Page, config.WEIGHTS_GG_COOKIE),
        puppeteerService.onStart(puppeteerService.loraSearchPage as Page, config.WEIGHTS_GG_COOKIE)
    ]);
    ...
    await puppeteerService.generationPage.exposeFunction(
        'handlePreviewUpdate', 
        (data: any) => previewHandler.handlePreviewUpdate(data)
    );
